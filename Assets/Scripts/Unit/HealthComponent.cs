...
{    
    ...
    public void GiveDamage(Unit target)
    {
        if (attackState == ReadyState.Ready)
        {
            ...
            RecordStatistics.Instance.WriteStatistic(_owner,RecordStatistics.action.DamageDealed, data.damage);
        }
    }
    ...
    public void HealthHeal(float Amount) { 
        _health.Heal(Amount);
        RecordStatistics.Instance.WriteStatistic(_owner, RecordStatistics.action.Healed, Amount);
    }
